...
from hippo_encoder.group_region import GroupRegionProgram, inside_fraction, soft_box_distance
...
def evaluate_case(
    ...
    program = GroupRegionProgram.from_teacher_spread(
    ...
    line_estimate = len(program.minus_groups) + len(program.plus_groups)
    token_estimate = sum(
        max(1, (len(f"minus {idx} {value:.2f}") + 3) // 4)
        for idx, value in enumerate(program.minus_groups)
    ) + sum(
        max(1, (len(f"plus {idx} {value:.2f}") + 3) // 4)
        for idx, value in enumerate(program.plus_groups)
    )

    return {
        "query": query,
        "num_groups": program.num_groups,
        "group_token_estimate": token_estimate,
        "group_line_estimate": line_estimate,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark grouped anchor-preserving region programs.")
