...
def encode_case_batch(
    ...
    query_texts = [row["query"] for row in batch]
    ...
    queries = outputs[:, 0]
    positives = outputs[:, 1 : 1 + num_pos]
    negatives = outputs[:, 1 + num_pos :]
    return queries, positives, negatives


def main() -> None:
    parser = argparse.ArgumentParser(description="Train the student formula head directly on IN/OUT behavior.")
    ...
    for epoch in range(args.num_epochs):
        for batch in loader:
            ...
            region = student.formula_head.hydrate_soft_region(
